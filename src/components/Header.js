...
import { prefix } from "../lib/prefix"; // <– note the relative path
      ...
      <div className="bg-[#0070C0] p-4 flex flex-col shadow-xl">
        <div className="flex flex-col md:flex-row items-center justify-between w-full">
          ...
          <div className="relative w-full max-w-xs md:mr-10 mt-4 md:mt-0">
            <img
              src={`${prefix}/images/searchicon.png`}
              alt="Search Icon"
            ...
            {menuItems.map((item, idx) => (
              ...
              >
                ...
                {item.submenu && activeSubmenu === item.label && (
                  ...
                  >
                    <div
                      className="p-4 space-y-2 font-bold"
                      style={{ fontFamily: "Open Sans, sans-serif" }}
                    >
                          ...
                          href={
                            item.label === "Information & Support"
                              ? subItem === "Prostate Cancer Information"
                                ? `${prefix}/information-support/prostate-cancer-info`
                                : subItem === "Risks & Symptoms"
                                ? `${prefix}/information-support/risks-symptoms`
                                : subItem === "Prostate Tests"
                                ? `${prefix}/information-support/prostate-tests`
                                : subItem === "Prevention"
                                ? `${prefix}/information-support/prevention`
                                : subItem === "Just Diagnosed"
                                ? `${prefix}/information-support/just-diagnosed`
                                : subItem === "Get Support"
                                ? `${prefix}/information-support/get-support`
                                : "#"
                              : item.label === "Learner Zone"
                              ? subItem === "Beginner's Guide"
                                ? `${prefix}/learner-zone/beginners-guide`
                                : subItem === "Interactive Zone"
                                ? `${prefix}/learner-zone/interactive-zone`
                                : "#"
                              : item.label === "News & Stories"
                              ? subItem === "Latest News"
                                ? `${prefix}/news-stories?category=Latest%20News`
                                : subItem === "Events"
                                ? `${prefix}/news-stories?category=Events`
                                : subItem === "Real Stories"
                                ? `${prefix}/news-stories?category=Real%20Stories`
                                : "#"
                              : item.label === "Research"
                              ? subItem === "Clinical Trials"
                                ? `${prefix}/research?category=Clinical%20Trials`
                                : subItem === "Latest Findings"
                                ? `${prefix}/research?category=Latest%20Findings`
                                : "#"
                              : "#"
                          }
